...
const OnboardingPage = () => {
  ...
  useEffect(() => {
    document.title = "Minha Obra | Nova obra";
    checkExistingObra();
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const { error: etapasError } = await supabase.from("etapas").insert(etapasParaInserir);
      ...
      <main className="mx-auto flex w-full max-w-md flex-1 flex-col justify-between px-4 pb-10 pt-8">
        <div>
          <header className="mb-8 space-y-2">
            ...
            <h1 className="text-2xl font-semibold leading-tight">Configure sua primeira obra</h1>
            <p className="text-sm text-muted-foreground">
              Em poucos passos você terá um painel visual para acompanhar orçamento, etapas e decisões
          ...
          <form
            ...
            className="space-y-5 card-elevated p-5 animate-scale-in"
          >
